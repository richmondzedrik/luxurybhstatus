...
const BossPage = () => {
  ...
  const { sendBossToDiscord, isConnected: isDiscordConnected, isBotReady } = useDiscordBot()

  const { getParticipationSummary } = useBossParticipation()
  const [forceUpdate, setForceUpdate] = useState(0)
  const [sendingToDiscord, setSendingToDiscord] = useState({})
  ...
  // Handle sending boss to Discord
  const handleSendToDiscord = async (boss) => {
    if (!isBotReady()) {
      alert('❌ Discord bot is not connected. Please configure the Discord bot in Admin settings.')
      return
    }

    const bossId = boss.id || boss.monster
    setSendingToDiscord(prev => ({ ...prev, [bossId]: true }))

    try {
      const result = await sendBossToDiscord(boss)

      if (result.success) {
        alert(`✅ Boss "${boss.monster}" sent to Discord successfully!`)
      } else {
        alert(`❌ Failed to send boss to Discord: ${result.error}`)
      }
    } catch (error) {
      console.error('Error sending boss to Discord:', error)
      alert(`❌ Error sending boss to Discord: ${error.message}`)
    } finally {
      setSendingToDiscord(prev => ({ ...prev, [bossId]: false }))
    }
  }
                  ...
                  <div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-600">
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-xs text-gray-600 dark:text-gray-400 mt-1">
                          Send this boss alert to Discord for team coordination
                        </p>
                      </div>
                      <button
                        onClick={() => handleSendToDiscord(boss)}
                        disabled={sendingToDiscord[boss.id || boss.monster] || !isBotReady()}
                      ...
                      >
                        {sendingToDiscord[boss.id || boss.monster] ? (
                          <>
                    ...
                    {!isBotReady() && (
                      <p className="text-xs text-red-500 dark:text-red-400 mt-2">
                        Discord bot not connected. Configure in Admin settings.
                      </p>
                    )}
                  </div>
